...
import type { GameState, ArenaEvent, ActionResult, BotDifficulty } from "@aura-ego/shared";

interface GameStore {
  roomId: string | null; state: GameState | null; event: ArenaEvent | null;
  evaluation: ActionResult | null; status: "IDLE" | "SEARCHING" | "FOUND" | "PLAYING" | "ENDED";
  ...
  setEvent: (event: ArenaEvent | null) => void; setEvaluation: (result: ActionResult) => void;
  setStatus: (status: GameStore["status"]) => void; nextSequence: () => number; reset: () => void;
}
export const useGame = create<GameStore>((set, get) => ({
  roomId: null, state: null, event: null, evaluation: null, status: "IDLE",
  ...
  reset: () => set({ roomId: null, state: null, event: null, evaluation: null, status: "IDLE", sequence: 0 })
}));
